# This is the source code of ADC 1.3.1, please handle it with care.
        ...
        with Bar("Extracting files...", max=file_count) as bar:
            for filename, compressed_data in files_to_extract:
                ...
                if decompressed_data is not None:
                    ...
                    print(
                        f"Skipping extraction for {filename} due to decompression error."
                    )
...
def save_archive_file():
    ...
    file_path = filedialog.asksaveasfilename(
        defaultextension=".adc", title="Save ADC archive as"
    )
    root.destroy()
    return file_path
...
while True:
    command = (
        input(
            "Welcome to the ADC Archiver! Enter command (c to create, e to extract, i for info, q to quit): "
        )
        .strip()
        .lower()
    )
    ...
    if command == "c":
     ...
     {RED}####        ####{reset}  {PURPLE}%%%%%%%%%{reset}          {GREEN}*******{reset}  
        ...
        | ADC Archiver | Version 1.3.1 | byte-key: 8 |
